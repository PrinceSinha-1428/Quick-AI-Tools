  ...
  return (
    <div className={`w-60 bg-white border-r border-gray-200 flex flex-col justify-between items-center max-sm:absolute top-14 bottom-0 ${sideBar ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
      <div className='my-7 w-full'>
        ...
        <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
            {navItems.map(({to,label,Icon}) => (
                <NavLink key={to} to={to} end={to === '/ai'} onClick={() => setSideBar(false)} className={({isActive}) => `px-3.5 py-2.5 flex items-center gap-3 rounded-full transition-all ease-in-out duration-200 ${isActive ? 'bg-gradient-to-r from-[#3C81F6] to-[#9234EA] text-white' : '' }`}>
                    {({isActive}) => (
                        <>
      ...
      <div className='w-full border-t border-gray-200 p-4 px-6 flex items-center justify-between'>
            ...
            <LogOut onClick={() => signOut()} className='cursor-pointer  w-4.5 text-gray-500 hover:text-gray-700 transition'/>
      </div>
    </div>
  )
}
